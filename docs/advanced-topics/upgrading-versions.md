...
---
# Upgrading Versions
The Dashboard Starter Kit is a Laravel package and is very well maintained. The package is regularly updated with new features and bug fixes.
To keep up with the latest version, you can run the update command.
```bash
composer update uneca/dashboard-starter-kit
```
Once the package has been updated, you might need to use the built-in update command to update the application's published resources.
The update command greatly aids in updating the version of Dashboard Starter Kit you are running.

> Note: You will not need to run this command unless it is specified in the release notes for the new version.

```bash
php artisan chimera:update [options]
```
It has the following options, which you might use depending on what is recommended in the release notes for each version.
...
```
::: warning
Please be aware that when you re-publish previously published resources, changes you might have made to those resources after installation will likely be overwritten. Under normal circumstances, you will likely not have modified any of these resources but be aware that this might happen.
